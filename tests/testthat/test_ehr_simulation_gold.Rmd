...
test_that("ehr_def is sane", {
    # Check if ehr_def object is of class EHR_definition
    expect_is(ehr_def, "EHR_definition")
    # Check if the start_date attribute of ehr_def object is of class Date
    expect_is(ehr_def$start_date, "Date")
    # Check if the end_date attribute of ehr_def object is of class Date
...
test_that("define_EHR is sane", {
    # Expect a warning when calling define_EHR() with no arguments
    expect_warning(define_EHR())
    # Check if the ed2 object has the expected named elements
    expect_named(ed2, c("start_date", "end_date", "patient", 
                                 "practice", "clinical", "referral", 
                                 "consultation", "therapy"), ignore.order = TRUE)
    # Check if the ed2 object is of class EHR_definition
    expect_is(ed2, "EHR_definition")
    # Check if the start_date attribute of ed2 is of class Date
    expect_is(ed2$start_date, "Date")
    # Check if the end_date attribute of ed2 is of class Date
    expect_is(ed2$end_date, "Date")
    # Check if the number of patients in ed2 matches the expected reduced_patients value
